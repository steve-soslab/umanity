...
export default async function handler(
  ...
  try {
    ...
    const race: UmanityRaceID = req.body;
    const raceids = new Array(10).fill("").map((_, i) => {
      return `${race.RaceDate}${race.TrackID}${i.toString()}${race.RaceNumber}`;
    });

    const promises = [];
    raceids.forEach((r) => {
      promises.push(fetch(`${url}${r}`));
    });
